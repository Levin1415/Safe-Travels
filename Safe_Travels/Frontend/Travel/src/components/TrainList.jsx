...
const TrainList = ({token}) => {
    ...
    // Train type themes
    const trainTypeThemes = {
        'Express': { bg: 'bg-blue-100', text: 'text-blue-800', border: 'border-blue-200' },
        'Superfast': { bg: 'bg-purple-100', text: 'text-purple-800', border: 'border-purple-200' },
        'Rajdhani': { bg: 'bg-red-100', text: 'text-red-800', border: 'border-red-200' },
        'Shatabdi': { bg: 'bg-green-100', text: 'text-green-800', border: 'border-green-200' },
        'Duronto': { bg: 'bg-yellow-100', text: 'text-yellow-800', border: 'border-yellow-200' },
        'Passenger': { bg: 'bg-gray-100', text: 'text-gray-800', border: 'border-gray-200' }
    }

    // Enhanced Special offers with images
    const specialOffers = [
        {
            id: 1,
            title: "TATKAL BOOKING OPEN",
            subtitle: "Book Tatkal tickets with 0% convenience fee",
            description: "Special fares for last-minute bookings",
            icon: "⚡",
            image: "https://images.unsplash.com/photo-1519457431-44ccd64a579b?w=800&auto=format&fit=crop&q=60",
            gradient: "linear-gradient(135deg, #f59e0b 0%, #dc2626 100%)",
            badge: "HOT DEAL",
            buttonText: "Book Now",
            features: ["Instant Confirmation", "Zero Convenience Fee", "Priority Waiting List"]
        },
        {
            id: 2,
            title: "FIRST JOURNEY OFFER",
            subtitle: "Get ₹200 off on your first train booking",
            description: "Welcome offer for new travelers",
            icon: "🎁",
            image: "https://images.unsplash.com/photo-1527266237111-a4981f9c1463?w-800&auto=format&fit=crop&q=60",
            gradient: "linear-gradient(135deg, #3b82f6 0%, #06b6d4 100%)",
            badge: "NEW USER",
            buttonText: "Claim Offer",
            features: ["₹200 Instant Discount", "Free Cancellation", "Priority Seats"]
        },
        {
            id: 3,
            title: "WEEKEND ESCAPE",
            subtitle: "Special discounts on weekend travel",
            description: "Perfect for your weekend getaway",
            icon: "🏖️",
            image: "https://images.unsplash.com/photo-1581094794329-c8112a89af12?w=800&auto=format&fit=crop&q=60",
            gradient: "linear-gradient(135deg, #10b981 0%, #059669 100%)",
            badge: "WEEKEND",
            buttonText: "Explore",
            features: ["Up to 30% Off", "Flexible Dates", "Family Packages"]
        },
        {
            id: 4,
            title: "GROUP DISCOUNT",
            subtitle: "Extra 25% off for groups of 4+",
            ...
            features: ["25% Group Discount", "Seats Together", "Special Coach"]
        },
        {
            id: 5,
            ...
            features: ["Up to 40% Savings", "Best Seats", "Free Meal"]
        }
    ]
    ...
    useEffect(() => {
        const fetchTrains = async() => {
            try {
                ...
                extractStations(response.data)
                calculatePopularRoutes(response.data)
            } catch (error) {
                console.log('error in fetching trains', error)
    ...
    const calculateStats = (trainsData) => {
        const totalTrains = trainsData.length
        const availableSeats = trainsData.reduce((total, train) => {
            const seats = train.seats ? train.seats.filter(s => !s.is_booked).length : 0
            return total + seats
        }, 0)
        const stationsCovered = new Set(trainsData.flatMap(train => [train.origin, train.destination])).size
    ...
    const extractStations = (trainsData) => {
        const allStations = new Set()
        trainsData.forEach(train => {
            allStations.add(train.origin)
            allStations.add(train.destination)
        })
        setAvailableStations(Array.from(allStations).sort())
    }

    // Calculate popular routes
    const calculatePopularRoutes = (trainsData) => {
        const routeCount = {}
        trainsData.forEach(train => {
    ...
    useEffect(() => {
        ...
        if (filters.trainType) {
            filtered = filtered.filter(train => 
                train.train_type.toLowerCase().includes(filters.trainType.toLowerCase())
            )
        }
    ...
    const getTrainTheme = (trainType) => {
        const type = Object.keys(trainTypeThemes).find(key => 
            trainType.toLowerCase().includes(key.toLowerCase())
        )
        return trainTypeThemes[type] || trainTypeThemes['Passenger']
    }
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center">
                <div className="text-center space-y-4">
                    ...
                    <p className="text-gray-600 font-medium animate-pulse">Loading train schedules...</p>
                </div>
            </div>
        )
    }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
            ...
            <div className="relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-r from-blue-500/5 to-purple-500/5 transform -skew-y-3"></div>
                <div className="relative max-w-7xl mx-auto px-4 py-12">
                    <div className="text-center mb-8">
                        <h1 className="text-5xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent mb-4">
                            Indian Railways
                        </h1>
                        <p className="text-gray-600 text-lg">Journey across India with comfort and speed</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
                        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    <div className="text-2xl font-bold text-gray-800">{stats.totalTrains}</div>
                                    <div className="text-gray-500 text-sm">Active Trains</div>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    <div className="text-2xl font-bold text-gray-800">{stats.stationsCovered}</div>
                                    <div className="text-gray-500 text-sm">Stations Covered</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="max-w-7xl mx-auto px-4 pb-16">
                ...
                <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-sm p-6 mb-8 border border-gray-200/50">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-800">Find Your Train</h2>
                            <p className="text-gray-500 text-sm mt-1">Search across {stats.totalTrains} trains</p>
                        </div>
                        {(filters.origin || filters.destination || filters.trainType) && (
                            <button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                list="stations-destination"
                                placeholder="To station"
                                value={filters.destination}
                                onChange={(e) => setFilters({...filters, destination: e.target.value})}
                                className="w-full pl-10 pr-4 py-3 border border-gray-300/50 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-transparent"
                            />
                            <datalist id="stations-destination">
                                {availableStations.map(station => (
                                    <option key={station} value={station} />
                                ))}
                            </datalist>
                        ...
                        <input
                            type="text"
                            placeholder="Train type (Express, Superfast...)"
                            ...
                            {popularRoutes.map((route) => (
                                ...
                                >
                                    <div className="text-center">
                                        ...
                                        <div className="text-xs text-gray-500 mt-1">{route.trains} trains</div>
                                    </div>
                                </button>
                            ))}
                        </div>
                ...
                <div className="mb-12">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-2xl font-bold text-gray-800">Exclusive Offers</h3>
                ...
                <div className="mb-12">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xl font-bold text-gray-800">
                            Available Trains {filteredTrains.length > 0 && `(${filteredTrains.length})`}
                        </h3>
                    ...
                    <div className="space-y-4">
                        {filteredTrains.map((train) => {
                            const availableSeats = train.seats ? train.seats.filter(s => !s.is_booked).length : 0
                            const duration = calculateDuration(train.start_time, train.reach_time)
                            const theme = getTrainTheme(train.train_type)
                                        ...
                                        <div className="flex flex-col lg:flex-row lg:items-center justify-between">
                                            ...
                                            <div className="flex-1 mb-6 lg:mb-0 lg:pr-8">
                                                <div className="flex items-start justify-between mb-4">
                                                    <div>
                                                        <div className="flex items-center space-x-3 mb-2">
                                                            <h4 className="text-xl font-bold text-gray-800">{train.train_name}</h4>
                                                        ...
                                                        <div className="flex items-center space-x-4">
                                                            ...
                                                            <span className="text-sm text-gray-500">Coach: {train.coach_type}</span>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <div className="text-2xl font-bold text-green-600">₹{train.price}</div>
                                                ...
                                                <div className="flex items-center space-x-6">
                                                    <div className="text-center">
                                                        <div className="font-bold text-gray-800">{train.origin}</div>
                                                        <div className="text-sm text-blue-600">{train.start_time}</div>
                                                    </div>
                                                    <div className="flex-1 relative">
                                                        <div className="h-px bg-gradient-to-r from-blue-200 to-purple-200"></div>
                                                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                                                            <div className="bg-white border border-gray-300/50 rounded-full p-2">
                                                                <span className="text-xs text-gray-600">{formatDuration(duration)}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div className="text-center">
                                                        <div className="font-bold text-gray-800">{train.destination}</div>
                                                        <div className="text-sm text-green-600">{train.reach_time}</div>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="lg:border-l lg:border-gray-200/50 lg:pl-8">
                                                <button
                                                    ...
                                                    className={`w-full lg:w-auto px-8 py-3 text-white font-medium rounded-xl hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5 ${
                                                        availableSeats > 0 
                                                        ? 'bg-gradient-to-r from-blue-600 to-purple-600 hover:shadow-blue-100' 
                        ...
                        {filteredTrains.length === 0 && trains.length > 0 && (
                            <div className="text-center py-16">
                                <div className="w-24 h-24 mx-auto mb-6 bg-gradient-to-br from-gray-100 to-gray-200 rounded-full flex items-center justify-center">
                                    <span className="text-4xl">🚂</span>
                                </div>
                                <h4 className="text-xl font-semibold text-gray-700 mb-2">No trains found</h4>
                        ...
                        {trains.length === 0 && (
                            <div className="text-center py-16">
                                ...
                                <h4 className="text-xl font-semibold text-gray-700 mb-2">No trains available</h4>
                                <p className="text-gray-500">Check back later for train schedules</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <footer className="pt-8 border-t border-gray-200/50">
                    <div className="text-center">
                        ...
                        <p className="text-gray-500 text-sm mb-6">
                            Lifeline of the nation, journey with pride
                        </p>
                        <div className="flex items-center justify-center space-x-6 mb-6">
                            <span className="text-xs text-gray-400">🚅 Safety First</span>
                            <span className="text-xs text-gray-400">🎫 Easy Booking</span>
                            <span className="text-xs text-gray-400">⏱️ On Time</span>
                        </div>
